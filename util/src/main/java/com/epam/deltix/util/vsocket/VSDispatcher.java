    ...
    public void                 close(boolean wait) {
        if (wait) {
            if (getActiveChannels() > 0)
                synchronized (channels) {
                    try {
                        channels.wait(VSProtocol.SHUTDOWN_TIMEOUT);
                    } catch (InterruptedException e) {
                        if (VSProtocol.LOGGER.isLoggable(Level.FINE))
...
//            }
    ...
    private void                sendClosing() {
        ...
        try {
            ...
            DataExchangeUtils.writeUnsignedShort(buffer, 0, VSProtocol.DISPATCHER_CLOSE);
            channel = checkOut();
    ...
    VSChannelImpl               newChannel (int inCapacity, int outCapacity, boolean compressed) {
        ...
        synchronized (channels) {
            ...
            if (localId >= VSProtocol.LISTENER_ID)
        ...
        synchronized (channels) {
            if (id >= channels.size() ) {
                ...
                if (VSProtocol.LOGGER.isLoggable(Level.WARNING)) {
                    VSProtocol.LOGGER.warning("Attempt to use invalid channel id: " + id + ", max valid value: " + (channels.size() - 1));
                }
                return null;
            }
