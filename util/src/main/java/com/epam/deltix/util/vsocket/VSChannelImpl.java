...
import com.epam.deltix.util.io.CountingInputStream;
        ...
        synchronized (this) {
            ...
            try {
                ...
                if (state != VSChannelState.NotConnected)
                    ...
                    VSProtocol.LOGGER.log(Level.FINE, this + " not connected yet");
...
//    }
    ...
    void                    receive(long position, byte[] data, int offset, int length, int code, int socketNumber) throws IOException {
        ...
        try {
            ...
            if (VSProtocol.LOGGER.isLoggable(Level.FINEST)) {
                VSProtocol.LOGGER.log(Level.FINEST, "Got data block " + position + ":" + (position + unpackedLength) + " (" + length + "/" + unpackedLength + ") from socket: @" + Integer.toHexString(code) + "#" + socketNumber);
            }
        } catch (EOFException e) {
            // ignore
        } finally {
        ...
        throws InterruptedException, IOException
    {
        ...
        if (state != VSChannelState.Connected && state != VSChannelState.RemoteClosed) {
            if (VSProtocol.LOGGER.isLoggable(Level.FINE))
                VSProtocol.LOGGER.log(Level.FINE, "Skipping BYTES_AVAILABLE_REPORT report: " + bytes + " because channel state is " + state);
            return;
        }

        state = getState();

        if (state == VSChannelState.RemoteClosed && VSProtocol.LOGGER.isLoggable(Level.FINE))
            VSProtocol.LOGGER.log(Level.FINE, "Sending BYTES_AVAILABLE_REPORT report: " + bytes + " at state " + state + " with remoteIndex=" + remoteIndex);
        ...
        if (VSProtocol.LOGGER.isLoggable(Level.FINEST))
            VSProtocol.LOGGER.log(Level.FINEST, "Sending BYTES_AVAILABLE_REPORT report: " + ((int)bytes) + " from " + tc.getSocketIdStr());
        ...
        throws InterruptedException, IOException
    {
        ...
        DataExchangeUtils.writeUnsignedShort (data, 0, VSProtocol.LISTENER_ID);
        ...
        throws InterruptedException, IOException
    {
        ...
        DataExchangeUtils.writeUnsignedShort (data, 2, VSProtocol.CLOSING);
        ...
        if (VSProtocol.LOGGER.isLoggable(Level.FINEST))
            VSProtocol.LOGGER.log(Level.FINEST, "Sending CLOSING: remoteIndex=" + remoteIndex + " numBytesSend=" + numBytesSend);
    }
        ...
        throws InterruptedException, IOException
    {
        ...
        DataExchangeUtils.writeUnsignedShort (data, 0, remoteId);
        DataExchangeUtils.writeUnsignedShort (data, 2, VSProtocol.CLOSED);
        DataExchangeUtils.writeInt (data, 4, remoteIndex);
        DataExchangeUtils.writeLong(data, 8, numBytesSend);
        ...
        if (VSProtocol.LOGGER.isLoggable(Level.FINEST))
            VSProtocol.LOGGER.log(Level.FINEST, "Sending CLOSED: remoteIndex=" + remoteIndex + " numBytesSend=" + numBytesSend);
    }
        ...
        throws InterruptedException, IOException
    {
        ...
        DataExchangeUtils.writeUnsignedShort (buffer8, 2, VSProtocol.BYTES_AVAILABLE_REPORT);
        ...
        DataExchangeUtils.writeUnsignedShort (data, 0, remoteId);
        DataExchangeUtils.writeUnsignedShort (data, 2, VSProtocol.CONNECT_ACK);
    ...
    boolean assertIndexValid(int dataSize, int incoming) {
        ...
        if (!valid)
            VSProtocol.LOGGER.log (Level.SEVERE,
